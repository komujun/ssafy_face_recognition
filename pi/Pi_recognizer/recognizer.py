...
connection=pymysql.connect(host='i3a207.p.ssafy.io',user='a207',password='a207',db='mydbtest2',charset='utf8',autocommit=True)
...
while True:
    ...
    for(x,y,w,h) in faces:
        ...
        if (confidence < 90): # confidence = 0 is perfect match 
            ...
            if ((plist[idno]["out_at"] is not None) and ((nowtime-plist[idno]["out_at"]).seconds>=10)):
                plist[idno].update(enter_at = nowtime)
                plist[idno].update(out_at=None)
                cursor.execute("INSERT blog_access SET user_pk_id= %s, enter_at = %s",(idno,nowtime))
                print("Enter: "+id)
            elif((plist[idno]["out_at"] is None) and (nowtime-plist[idno]["enter_at"]).seconds>=10):
                plist[idno].update(out_at = nowtime)
                cursor.execute("UPDATE blog_access SET out_at = %s WHERE user_pk_id = %s ORDER BY enter_at DESC LIMIT 1",(nowtime,idno))
                print("Out  : "+id)
                print(nowtime.day)
